...
export const connectWebSocket = async (onKeysRefreshed) => {
    try {
                        ...
                        Alert.alert(
                            ...
                            [
                                { text: 'Ignore', style: 'cancel' },
                                {
                                    text: 'Sync Now',
                                    onPress: async () => {
                                        await saveSession(session.user, data.anonKey, data.secretKey);
                                        if (onKeysRefreshed) onKeysRefreshed(data.anonKey, data.secretKey);
                                        Alert.alert('Done!', 'Keys synced!');
                                    }
                                }
                            ]
                        );
        ...
        ws.onclose = () => {
            ws = null;
            if (shouldReconnect) {
                reconnectTimer = setTimeout(() => connectWebSocket(onKeysRefreshed), 5000);
            }
        };
    } catch (e) { console.log('WS error:', e.message); }
};
